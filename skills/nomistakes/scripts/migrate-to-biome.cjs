...
function findRootDir() {
  ...
  while (dir !== '/' && iterations < MAX_ITERATIONS) {
...
function findExistingConfigs(rootDir) {
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      warning(`Could not parse package.json: ${e.message}`)
    }
  }
...
async function main() {
  ...
  if (configs.eslint.length > 0 || configs.prettier.length > 0) {
    ...
    success(
      `Backed up ${backedUp.length} file(s) to ${path.relative(rootDir, backupDir)}/`,
    )
  ...
  const { removedFields } = updatePackageJson(rootDir)
  ...
  if (configs.eslint.length > 0 || configs.prettier.length > 0) {
    ...
    log(
      '  .biome-migration-backup/ (safe to delete after verification)',
      'gray',
    )
  }
