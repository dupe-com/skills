...
function enhanceContrast(samplingVector: number[], exponent: number): number[] {
  ...
  return samplingVector.map((v) => {
    const norm = v / maxVal;
    return Math.pow(norm, exponent) * maxVal;
  });
}
...
async function loadImage(filePath: string): Promise<ImageData> {
  try {
    ...
    throw new Error(
      "Could not load image. Please install sharp: bun add sharp"
    );
  }
}
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (dist < radius) {
        ...
        const dot = normalX * lightDir.x + normalY * lightDir.y + normalZ * lightDir.z;
